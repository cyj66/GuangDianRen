  ...
  <div class="login">
    ...
    <form action="/register" method="post">
      <ul>
        ...
        <p>
          已有账号?去<router-link to="/login"><span>登录</span></router-link>
        </p>
      </ul>
    </form>
  </div>
...
<script>
import { postUserRegister, getUserData } from "network/userRequest.js";
    ...
    registerSubmit() {
      getUserData().then((userList) => {
        ...
        if (!userList.some((value) => value.username === this.usernameValue)) {
          ...
          postUserRegister(this.usernameValue, this.passwordValue).then((res) =>
